...
export class BatchGeocodingService {
  ...
  async getGeocodingStats(): Promise<BatchGeocodingResult> {
    try {
      ...
      const propertiesWithoutCoordinates =
        totalProperties - propertiesWithCoordinates
  ...
  async batchGeocodeAllProperties(
    ...
    try {
      ...
      const propertiesForGeocoding = propertiesWithoutCoordinates.map(
        (property: {
          id: string
      ...
      for (let i = 0; i < propertiesForGeocoding.length; i += batchSize) {
        ...
        console.log(
          `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(propertiesForGeocoding.length / batchSize)}`
        )

        const batchResult =
          await this.coordinateService.batchGeocodeProperties(batch)
   ...
   */
  async getPropertiesNeedingGeocoding(limit: number = 100): Promise<
    Array<{
      id: string
      street_address: string
      city: string
