...
export class PropertyReconciliationService {
  ...
  async findMatchingProperty(
    ...
    for (const candidate of candidates) {
      const similarity = this.calculateAddressSimilarity(address, candidate.street_address);
   ...
   */
  async createNewProperty(propertyData: PropertyData, ownerId: string): Promise<Property> {
  ...
  async processProperty(
    ...
    if (match) {
      if (match.confidence >= 0.95) {
        ...
        const mergedProperty = await this.mergePropertyData(match.property, propertyData);
