import { BaseGeocodingClient, GeocodingRequest, GeocodingResult } from './geocoding-client';
...
export class GoogleGeocodingClient extends BaseGeocodingClient {
  ...
  private determineConfidence(types: string[]): 'high' | 'medium' | 'low' {
    ...
    if (types.some(type => ['street_address', 'premise', 'subpremise'].includes(type))) {
      return 'high';
    }
    
    // Medium confidence: route, intersection, neighborhood
    if (types.some(type => ['route', 'intersection', 'neighborhood'].includes(type))) {
      return 'medium';
    }
