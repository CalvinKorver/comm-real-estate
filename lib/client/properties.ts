...
export async function getOwnerWithProperties(
  ownerId: string,
  baseUrl?: string
) {
  try {
    ...
    if (typeof window === "undefined") {
      ...
      if (!absBase) {
        throw new Error(
          "Base URL is required for server-side fetches. Set NEXT_PUBLIC_BASE_URL in your environment."
        )
      }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch owner with properties: ${response.status} ${response.statusText}`
      )
    }
    const ownerWithProperties = await response.json()
    console.log(
      "Client: Successfully fetched owner with properties:",
      ownerWithProperties
    )
