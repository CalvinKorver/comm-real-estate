import {
  ConflictResolution,
  OwnerData,
  OwnerDeduplicationService,
  OwnerMatch,
} from "@/lib/services/owner-deduplication"
...
describe("Owner Deduplication Service", () => {
  ...
  describe("normalizeName", () => {
    it("should normalize names correctly", () => {
      const testCases = [
        ...
        {
          input: "A Ashenbrenner & M Suzanna",
          expected: "a ashenbrenner  m suzanna",
        },
      ]
  ...
  describe("calculateNameSimilarity", () => {
    it("should return 1.0 for identical names", () => {
      expect(service.calculateNameSimilarity("John Smith", "John Smith")).toBe(
        1.0
      )
      expect(service.calculateNameSimilarity("john smith", "JOHN SMITH")).toBe(
        1.0
    ...
    it("should calculate similarity for similar names", () => {
      ...
      expect(service.calculateNameSimilarity("John Smith", "J. Smith")).toBe(
        0.5
      )
      expect(service.calculateNameSimilarity("John Smith", "John Smyth")).toBe(
        0.5
      )
    })
    ...
    it("should handle special characters and variations", () => {
      ...
      expect(
        service.calculateNameSimilarity("John & Jane", "John Jane")
      ).toBeCloseTo(0.67, 1)
    })

    it("should handle LLC and business names", () => {
      expect(
        service.calculateNameSimilarity(
  ...
  describe("findPotentialDuplicates", () => {
    const mockOwner = {
      ...
      contacts: [{ phone: "206-555-0101", email: "john@email.com" }],
    }
  ...
  describe("resolvePhoneConflicts", () => {
    ...
    it("should return merge for high confidence matches", async () => {
      ...
      const result = await service.resolvePhoneConflicts(
        ownerData,
        existingOwners
      )
