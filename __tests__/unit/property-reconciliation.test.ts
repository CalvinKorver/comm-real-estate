...
} from "@/lib/services/property-reconciliation"
...
describe("Property Reconciliation Service", () => {
  ...
  describe("calculateAddressSimilarity", () => {
    it("should return 1.0 for identical addresses", () => {
      expect(
        service.calculateAddressSimilarity("123 Main St", "123 Main St")
      ).toBe(1.0)
      expect(
        service.calculateAddressSimilarity("123 MAIN ST", "123 Main St")
      ).toBe(1.0)
    ...
    it("should calculate similarity for similar addresses", () => {
      expect(
        service.calculateAddressSimilarity("123 Main St", "123 Main Street")
      ).toBe(1.0)
      expect(
        service.calculateAddressSimilarity("123 Main St", "123 Main St.")
      ).toBe(1.0)
    ...
    it("should handle different house numbers", () => {
      expect(
        service.calculateAddressSimilarity("123 Main St", "124 Main St")
      ).toBeCloseTo(0.58, 1)
      expect(
        service.calculateAddressSimilarity("123 Main St", "125 Main St")
      ).toBeCloseTo(0.58, 1)
    })

    it("should handle partial matches", () => {
      expect(
        service.calculateAddressSimilarity("123 Main St", "123 Main")
      ).toBeCloseTo(0.83, 1)
      expect(
        service.calculateAddressSimilarity("123 Main St", "Main St")
      ).toBeCloseTo(0.67, 1)
    })

    it("should handle completely different addresses", () => {
      expect(
        service.calculateAddressSimilarity("123 Main St", "456 Oak Ave")
      ).toBeCloseTo(0.25, 1)
      expect(
        service.calculateAddressSimilarity("123 Main St", "789 Pine Rd")
      ).toBeCloseTo(0.25, 1)
    })
    ...
    it("should handle complex address variations", () => {
      expect(
        service.calculateAddressSimilarity("123 Main Street", "123 Main St")
      ).toBe(1.0)
      expect(
        service.calculateAddressSimilarity(
          "123 Main St, Seattle",
          "123 Main St Seattle"
        )
      ).toBe(1.0)
  ...
  describe("findMatchingProperty", () => {
    ...
    it("should find exact address match", async () => {
      ...
      const result = await service.findMatchingProperty(
        address,
        city,
        zip,
        state
    ...
    it("should return null when fuzzy match confidence is too low", async () => {
      ...
      const candidates = [{ ...mockProperty, street_address: "123 Main St" }]
