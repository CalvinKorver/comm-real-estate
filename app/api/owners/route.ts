...
export async function GET(request: Request) {
  try {
    ...
    console.log(`API: Fetched ${result.owners.length} owners (page ${result.pagination.currentPage}/${result.pagination.totalPages})`)
    
    return NextResponse.json(result)
  } catch (error) {
    console.error('API: Error fetching owners:', error)
    return NextResponse.json(
      { error: 'Failed to fetch owners', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof Error && error.message === 'First name and last name are required') {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: 'Failed to create owner', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
} 