...
const statusIcon = (status: string) => {
  ...
  if (status === "fail") {
    return (
      <span title="Invalid" className="text-red-600">
        ❌
      </span>
    )
  }
  return (
    <span title="Unknown" className="text-yellow-500">
      ❓
    </span>
  )
}
...
const PropertyListItem = forwardRef<HTMLDivElement, PropertyListItemProps>(
  ({ property, selected, onClick, onPropertyUpdated }, ref) => {
    ...
    const phoneContacts = sortContactsByPriority(
      property.owners?.flatMap(
        (owner) => owner.contacts?.filter((c) => c.phone) || []
      ) || []
    ).slice(0, 3)
                    ...
                    {displayNotes.length === 0 ? (
                      ...
                      displayNotes.map((note, idx) => (
                        <TableRow key={note.id}>
                          <TableCell className="">{note.content}</TableCell>
                          <TableCell className="text-muted-foreground">
                            {note.created_at ? (
                              <Tooltip>
